...
const updateBill = () => {
    ...
    billPrice.innerHTML = `${totalBill.toFixed(2)}лв.`;
}
...
const setupEvents = (name) => {
    ...
    const dedleteBtn= document.querySelectorAll(`.${name} .sm-delete-btn`);
    ...
    counts.forEach((item, i) => {
        ...
        counterMinus[i].addEventListener('click', () => {
            if(item.innerHTML > 1){
                item.innerHTML--;
                totalBill -= cost;
                price[i].innerHTML = `${item.innerHTML * cost}лв.`;
                if(name == 'cart'){
                    updateBill();
                }
